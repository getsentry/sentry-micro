 ...
 *
 *   The block below is ISRG Root X1, the Let's Encrypt root that signs the public Sentry
 *   ingest TLS certs, so it verifies `*.ingest.sentry.io` out of the box.
 *
 ...
 *   renewal trade is exactly why the SDK leaves verification off by default.
 *
 * It is `static const`, so it lives in flash, not RAM — 1,939 bytes (1,391 bytes DER once
...
/* ISRG Root X1 — https://letsencrypt.org/certs/isrg-root-x1.pem */
